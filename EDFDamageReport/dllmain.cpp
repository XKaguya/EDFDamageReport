...
void PreInit()
{
    g_SignatureTable = {
		...
		"48 8B C4 48 89 58 ?? 55 56 57 41 54 41 55 41 56 41 57 48 8D A8 ?? ?? ?? ?? 48 81 EC 20 02 00 00 0F 29 70 ?? 0F 29 78 ?? 4C 8B EA 4C 8B F1 80 B9 ?? ?? ?? ?? 00 74 15 C6 81 ?? ?? ?? ?? 00 80 B9 ?? ?? ?? ?? 00 74 2A", // UpdateRadar
    };
    ...
    char buf[256];
    snprintf(buf, 256, "[HOOK] Pointer based on signatures: 0x%p 0x%p 0x%p 0x%p 0x%p",
